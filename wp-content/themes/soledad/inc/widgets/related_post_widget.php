	...
	class penci_related_news_widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if( is_singular( 'post' ) && ! empty( $args ) ) {
				...
				<?php
				$attrstyle = '';
				if( $ptfsfe ){ $attrstyle .= '.widget ul#penci-relatedwg-'. $rand .' li.featured-news .side-item .side-item-text h4 a{ font-size: '. $ptfsfe .'px; }'; }
				if( $ptfs ){ $attrstyle .= '.widget ul#penci-relatedwg-'. $rand .' li:not(.featured-news) .side-item .side-item-text h4 a{ font-size: '. $ptfs .'px; }'; }
				if( $pmfs ){ $attrstyle .= '.widget ul#penci-relatedwg-'. $rand .' li .side-item .side-item-text .side-item-meta{ font-size: '. $pmfs .'px; }'; }
				if( $image_type == 'horizontal' ){
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => esc_html__('Related Posts', 'soledad'), 'ptfsfe' => '', 'ptfs' => '', 'pmfs' => '', 'image_type' => 'default', 'type' => 'categories', 'orderby' => 'date', 'order' => 'DESC', 'title_length' => '', 'number' => 5, 'offset' => '', 'featured' => false, 'allfeatured' => false, 'thumbright' => false, 'twocolumn' => false, 'featured2' => false, 'postdate' => false, 'icon' => false );
