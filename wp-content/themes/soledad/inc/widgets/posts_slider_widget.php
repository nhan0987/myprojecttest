	...
	class penci_slider_posts_news_widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if ($loop->have_posts()) :
				...
				$rand = rand(100,10000);
				?>
				<div id="penci-postslidewg-<?php echo sanitize_text_field( $rand ); ?>" class="penci-owl-carousel penci-owl-carousel-slider penci-widget-slider penci-post-slider-<?php echo $style; ?>" data-lazy="true" data-auto="<?php echo $autoplaydata; ?>">
					<?php while ($loop->have_posts()) : $loop->the_post(); ?>
						<div class="penci-slide-widget">
							<div class="penci-slide-content">
								<?php if( $style != 'style-3' ) {?>
									<?php if( ! get_theme_mod( 'penci_disable_lazyload_layout' ) ) { ?>
									<span class="penci-image-holder <?php echo penci_classes_slider_lazy(); ?>" data-src="<?php echo penci_get_featured_image_size( get_the_ID(), penci_featured_images_size() ); ?>" title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></span>
									<?php } else { ?>
			...
			$attrstyle = '';
			if( $ptfs ){ $attrstyle .= '#penci-postslidewg-'. $rand .' .penci-widget-slide-detail h4 a{ font-size: '. $ptfs .'px; }'; }
			if( $pmfs ){ $attrstyle .= '#penci-postslidewg-'. $rand .' .penci-widget-slide-detail .slide-item-date{ font-size: '. $pmfs .'px; }'; }
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => esc_html__('Posts Slider', 'soledad'), 'autoplay' => false, 'ptfs' => '', 'pmfs' => '', 'image_type' => 'default', 'number' => 5, 'type' => 'latest', 'categories' => '', 'date' => false, 'style' => 'style-1' );
			...
			<p>
				...
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'autoplay' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'autoplay' ) ); ?>" <?php checked( (bool) $instance['autoplay'], true ); ?> />
			</p>
