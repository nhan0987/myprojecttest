...
if( ! class_exists( 'penci_popular_news_widget' ) ) {
	class penci_popular_news_widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			<?php
			$attrstyle = '';
			if( $ptfsfe ){ $attrstyle .= '.widget ul#penci-popularwg-'. $rand .' li.featured-news .side-item .side-item-text h4 a{ font-size: '. $ptfsfe .'px; }'; }
			if( $ptfs ){ $attrstyle .= '.widget ul#penci-popularwg-'. $rand .' li:not(.featured-news) .side-item .side-item-text h4 a{ font-size: '. $ptfs .'px; }'; }
			if( $pmfs ){ $attrstyle .= '.widget ul#penci-popularwg-'. $rand .' li .side-item .side-item-text .side-item-meta{ font-size: '. $pmfs .'px; }'; }
			if( $image_type == 'horizontal' ){
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['ordernum']    = $new_instance['ordernum'];
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => esc_html__('Popular Posts', 'soledad'), 'ptfsfe' => '', 'ptfs' => '', 'pmfs' => '', 'image_type' => 'default', 'type' => '', 'title_length' => '', 'number' => 5, 'offset' => '', 'categories' => '', 'featured' => false, 'allfeatured' => false, 'thumbright' => false, 'twocolumn' => false, 'featured2' => false, 'ordernum' => false, 'postdate' => false, 'icon' => false );
