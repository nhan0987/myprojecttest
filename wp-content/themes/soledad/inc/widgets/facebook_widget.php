...
if( ! class_exists( 'penci_facebook_widget' ) ) {
	class penci_facebook_widget extends WP_Widget {
			...
			if( $page_url ){
				?>		
					...
					<div class="fb-page" data-href="<?php echo esc_url( $page_url ); ?>"<?php if( $stream && is_numeric( $height ) && $height > 69 ): ?> data-height="<?php echo absint( $height ); ?>"<?php endif;?> data-small-header="false" data-hide-cover="<?php if($cover) { echo 'true'; } else { echo 'false'; } ?>" data-show-facepile="<?php if($faces) { echo 'true'; } else { echo 'false'; } ?>" data-show-posts="<?php if($stream) { echo 'true'; } else { echo 'false'; } ?>" data-adapt-container-width="true">
						<div class="fb-xfbml-parse-ignore"><a href="<?php echo esc_attr( $page_url ); ?>"><?php if($title) { echo sanitize_text_field( $title ); } else { echo 'Facebook'; } ?></a></div>
					</div>
				<?php
			}
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => 'Follow Me', 'cover' => '', 'language' => '', 'faces' => 'on', 'page_url' => '', 'height' => '290', 'stream' => true);
			...
			<p>
				...
				<small style="display: block; clear: both; padding-left: 0;" class="description"><?php _e('Fill the language code to use on Facebook Page Box here. By default, the language will follow the site language. See more <a href="https://developers.facebook.com/docs/internationalization/" target="_blank">here</a>', 'soledad'); ?></small>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'language' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'language' ) ); ?>" placeholder="EG. de_DE" value="<?php echo esc_attr( $instance['language'] ); ?>" />
			</p>
