...
class Caching
{
  ...
  public static function get_cache_file_name()
  {
    ...
    $cache_mobile = apply_filters('flying_press_cache_mobile', false);
    $file_name .= $cache_mobile && wp_is_mobile() ? '-mobile' : '';

    // Remove ignored query string parameters and generate hash of the remaining
    $ignore_queries = [...self::$default_ignore_queries, ...$config['cache_ignore_queries']];
  ...
  public static function is_cacheable($content)
  {
    ...
    $known_queries = [
      ...self::$default_ignore_queries,
      ...$config['cache_ignore_queries'],
      ...self::$default_include_queries,
      ...apply_filters('flying_press_cache_include_queries', $config['cache_include_queries']),
    ];
  ...
  public static function count_pages($path)
  {
    ...
    foreach ($iterator as $fileInfo) {
      if ($fileInfo->isDir()) {
        $count++;
        $subfolderPath = $fileInfo->getPathname();
        $count += self::count_pages($subfolderPath);
      }
    }
  ...
  {
    if (isset($_COOKIE['fp_logged_in_roles']) && !is_user_logged_in()) {
      // Unset the cookie
      unset($_COOKIE['fp_logged_in_roles']);
      // Set the cookie to expire in the past so it will be deleted by the browser
      setcookie('fp_logged_in_roles', '', time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false);
