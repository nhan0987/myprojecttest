...
class OptimizeCss {
	...
	public function process() {
		...
		$excludes = array_merge(
			$default_excludes,
			Util\option_to_array( get_theme_mod( 'penci_speed_optimize_css_excludes' ) )
		);
	...
	public function optimize( Stylesheet $sheet ) {
		...
		if ( $sheet->is_google_fonts() && get_theme_mod( 'penci_speed_optimize_gfonts_inline' ) ) {
			$minify = true;
		}

		// Theme Icon Font
		if ( $this->delay_all_fonts( $sheet ) ) {
			$sheet->set_render( 'delay' );
			$minify = false;
		}
		...
		if ( get_theme_mod( 'penci_speed_optimize_css_to_inline' ) && ! $this->delay_all_fonts( $sheet ) ) {
			if ( ! $sheet->content ) {
