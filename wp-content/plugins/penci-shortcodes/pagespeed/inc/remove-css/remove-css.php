...
class RemoveCss {
	...
	public function process() {
		...
		foreach ( $this->stylesheets as $sheet ) {
			...
			if ( $sanitized_css ) {
				...
				$replacement = "<style id='soledad_pagespeed-{$sheet->id}'>" . $sanitized_css . '</style>';
	...
	public function setup_valid_sheets() {
		...
		$excludes = array_merge(
			$default_excludes,
			Util\option_to_array( get_theme_mod( 'penci_speed_remove_css_excludes' ) )
		);
	...
	public function get_allowed_selectors() {
		...
		return apply_filters(
			'soledad_pagespeed/allow_css_selectors',
			array_filter( $allowed_any ),
			$this
		);
	}
